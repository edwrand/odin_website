...
function Book(title, author, numpage, hasread) {
  this.title = title
  this.author = author
  this.numpage = numpage
  this.hasread = hasread
  this.info = function () {
    return title + 'by' + author + 'has' + numpage + 'pages,' + hasread
  }
}
const artofwar = new Book('Art of War', 'Shin Tzu', '284', 'has not read')
console.log(artofwar.info())
...
fetch(
  `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol2}&apikey=${apiKey2}`
)
  .then((response) => response.json())
  .then((data) => {
    // parse the data and display it in the HTML element
